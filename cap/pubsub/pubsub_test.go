...
import (
	...
	local_pubsub "github.com/libp2p/go-libp2p-pubsub"
...
func TestPubSub(t *testing.T) {
	...
	ps := (&pubsub.Server{TopicJoiner: gs}).PubSub()
	defer ps.Release()
...
func TestMessageCopy(t *testing.T) {
	...
	*/
	...
	joiner := pubsub.Router(c1.Bootstrap(ctx))
...
func newGossipSub(ctx context.Context) (*local_pubsub.PubSub, func()) {
	h := newTestHost()

	ps, err := local_pubsub.NewGossipSub(ctx, h)
	if err != nil {
		panic(err)
	}
